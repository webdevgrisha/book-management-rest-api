...
async function getBooksController(req: Request, res: Response, next: NextFunction) {
  ...
  ensureIsPositiveInt(limit, 'currPage');

  try {
    const bookData = await getBooksService({ userId: userId!, limit, currPage });
